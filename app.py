...
# Function to download CSV from GitHub (tracked via Git LFS)
@st.cache_data
def download_csv_from_github():
    url = 'https://raw.githubusercontent.com/sarthakdevil/movie-recomendation/main/cosine_similarity_matrix.csv'  # Replace with your GitHub raw URL
    response = requests.get(url)
    if response.status_code == 200:
        with open("cosine_similarity_matrix.csv", "wb") as f:
            f.write(response.content)
        return pd.read_csv("cosine_similarity_matrix.csv")
    else:
